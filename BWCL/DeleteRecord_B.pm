...
sub DeleteRecordForm {
    ...
    $r->print(
        qq{$table_id_field = $id_selected<br />
<label class="cent" for="delete_me">Delete?</label>
<input class="cent" type="checkbox" id="delete_me" name="delete_me" />
</p>
...
#XXX
#Note:  If I delete all full_assemblies below the full_assembly_list, that goes to a total of $0
#If I then add a full_assembly with the proper name, money returns, but full_assembly_full_assembly_list_id
#does not reappear in full_assembly.
...
sub DeleteRecord {
    ...
    if ( $table eq 'assemblies' ) {
        specified_error_message(
            $r,
            $arg_ref->{lang},
            "Para borrar este registro, primero se necesita borrar todos los registros incluidos bajo este registro en la tabla full_assembly (si existen) y luego todos los registros correspondientes en la tabla assemblies_parts",
            "To delete this record, it is neccesary to delete all of the records in full_assembly (if they exist) and then delete all of the corresponding records in assemblies_parts that this record refers to."
        );
        ...
        specified_error_message(
            $r,
            $arg_ref->{lang},
            "Si este registro es el ultimo registro correspondiente a la tabla full_assembly_list, el registro en la tabla full_assembly_list debe estar borrado tambien",
            "If this record is the last record corresponding to the full_assembly_list record, that record should also be deleted."
        );
        $arg_ref->{field_selected} = "full_assembly_list_name";
        my $full_assembly_name;
        my $full_assembly_id = $q->param("id_selected");
        ($full_assembly_name) = $dbh->selectrow_array("SELECT full_assembly_name FROM full_assembly WHERE full_assembly_id = $full_assembly_id;");
        $arg_ref->{field_value_selected} = $full_assembly_name;
        $arg_ref->{table_selected} = "full_assembly_list";
        ViewRecords($arg_ref);
    }
...
#    open( my $fh, '>>', "../../deletes-B_$arg_ref->{Database}{database}.sql" ) or die $!;
    ...
    $SQL = "DELETE FROM $table WHERE $table_id_field = $id_selected;";
    $r->print(qq{$SQL});
    my $ur = $dbh->do($SQL);
    if ($ur) {
...
This documentation refers to BWCL::DeleteRecord_B version 0.0.01.
