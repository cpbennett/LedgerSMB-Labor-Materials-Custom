...
our @ISA = qw(Exporter);
our @EXPORT_OK =
  qw(ShowTables ShowAllTables ShowColumns error_message specified_error_message);
...
sub error_message {
    ...
    if ( $lang eq "es" ) {
        $r->print(
qq{<div class="cent">
<p class="error">ERROR!! Por favor, selecciona $error_item_es.</p>
</div>}
        );
    }
    else {
        $r->print(
qq{<div class="cent">
<p class="error">ERROR!! Please select $error_item_en.</p>
</div>}
        );
    }
    return;
...
sub specified_error_message {
    ...
    if ( $lang eq "es" ) {
        $r->print(
qq{<div class="cent">
<p class="error">$specified_error_item_es</p>
</div>}
        );
    }
    else {
        $r->print(
qq{<div class="cent">
<p class="error">$specified_error_item_en</p>
</div>}
        );
    }
    return;
...
sub ShowColumns {
    ...
    my $col_name2;
    my $relid;
    my $showcolumnstable  = $arg_ref->{table};
...
#prints only column name,
#next step prints attributes(set for $ (0 .. etc to include column name
            $col_name  = "$tbl->[$i][0]";
...
#    $statement = "SELECT constraint_name, column_name, table_name
...
ShowTables Shows all the tables used in a database.
ShowAllTables Shows all of the tables in entire set of databases.
ShowColumns Shows the columns and some details for a particular table.
error_message Shows error message with a short specified section when called.
specified_error_message Shows error message that is only what is called with.
