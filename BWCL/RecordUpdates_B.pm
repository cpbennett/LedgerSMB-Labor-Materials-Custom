...
#	@field_names = @$field_names_aref;
    ...
    ( $field_names, $table_id_field )
        = Selecter( "RecordUpdates", $table );
    @field_names    = @$field_names;
    $table_id_field = $$table_id_field;
    my $table_id_field2 = $dbh->quote($table_id_field);
        ...
        if ( $class_selected ne 'All' ) {
            ...
            $statement = "
                   SELECT
                          ...
                          price
                     FROM $table
                    WHERE (class = $class_selected2
                      AND vendor_name = $vendor_name_selected2
            ...
            $statement = "
                   SELECT
                          ...
                          price
                     FROM $table
                    WHERE (vendor_name = $vendor_name_selected2
                      AND (sku, model, product_description)
                       IN
                            (
                                SELECT
