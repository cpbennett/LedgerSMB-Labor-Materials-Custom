...
sub ViewFullFALRecords {
    ...
    my @all_assemblies     = ();
    my @vetor              = ();
    my $full_assembly_list = $q->param("full_assembly_list_selected");
...
sub DuplicateFullFALRecords {
    ...
    $r->print(
          qq|<br />xxxxxxxxxxx ASSEMBLIES xxxxxxxxxxxxxxxxxxxxx<br />|);
    ...
    for my $i ( 1 .. $#assembly_insert_assemblys ) {
                 ...
                 || $assembly_insert_values[$i] =~ /Reuse/ )
        {
            ...
            if ($num_rows) {
                ...
                $r->print(
                         qq{<div><h1>Success - $new_ass_id</h1></div>});
            }
            else {
                $r->print(
    ...
    foreach my $key ( keys %args ) {
        ...
        {
            $quoted = $args{$key};
            push @{ $assemblies_parts_inserts[$2][$3] },
                { $1 => $quoted };
        ...
        for my $j ( 1 .. $#{ $assemblies_parts_inserts[$i] } ) {
            for my $k ( @{ $assemblies_parts_inserts[$i][$j] } ) {
                for my $key ( sort keys %{$k} ) {
                    $assemblies_parts_insert_assemblys[$i][$j]
                        .= "$key, ";
                    unless (    ( $key =~ /_id/ )
                             || ( $key =~ /quantity/ ) )
                    {
                    ...
                    $assemblies_parts_insert_values[$i][$j]
                        .= "$quoted, ";
                }
            }
            $assemblies_parts_insert_assemblys[$i][$j] =~ s/, $//;
            $assemblies_parts_insert_values[$i][$j]    =~ s/, $//;
            unless ( $assemblies_parts_insert_values[$i][$j]
                     =~ /DoNotInsert/ )
            {
                $SQL
...
This documentation refers to BWCL::ViewFALRecords version 3.0.00.

=head1 SYNOPSIS

Allows viewing existing full_assembly_lists both light in detail
and more fully.
Creates approximate copies of existing full_assembly_lists, allowing
