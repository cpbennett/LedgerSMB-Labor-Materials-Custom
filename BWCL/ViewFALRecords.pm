...
sub ViewFALRecords {
    ...
    my $dbh                         = $arg_ref->{dbh};
    my $q                           = $arg_ref->{q};
    my $full_assembly_list_selected = $arg_ref->{full_assembly_list};
       $full_assembly_list_selected =~ s/&quot;/"/g;
    my $full_assembly_list_selected2 =
      $dbh->quote($full_assembly_list_selected);
    my $assembly_selected = $arg_ref->{assembly};
...
#    $full_assembly_list_selected =~ s/''/"/g;
    ...
    for my $assembly_selected (@all_assemblies) {
        ...
        my @hash_assemblys = qw(
          assembly_part_description assembly_part_name
          assembly_part_class         assembly_part_subclass
    ...
    <caption class="black">
    <strong class="black">$assembly_name</strong>
    </caption>
    <thead>
        <tr>
...
# This substitution allows newlines to be converted to HTML code
                ...
                $r->print(
                    qq|<td align="left">$assem_part_hash_ref->{$assemblyh}</td>|
                );
            }
            $r->print(qq{</tr>});
        }
...
sub ViewFullFALRecords {
    ...
    my $assembly           = $arg_ref->{assembly};
    $assembly =~ s/&quot;/"/g;
    my $full_assembly_list = $arg_ref->{full_assembly_list};
...
#    $assembly           =~ s/''/"/g;
    ...
    $full_assembly_list = $dbh->quote($full_assembly_list);
              ...
              WHERE assembly_id
                 IN
                    (
                        SELECT full_assembly_assembly_id
                          FROM full_assembly
                         WHERE full_assembly_name=$full_assembly_list
                    )
           ORDER BY assembly_name;"
        );
        $sth->execute;
    ...
    <tbody>}
        ...
        while ( $assem_hash_ref = $sth->fetchrow_hashref ) {
            ...
            foreach my $assemblyh (@assem_hash_assemblys) {
                ...
                $r->print(
qq|<td align="left" class="assemblies_b">$assem_hash_ref->{$assemblyh}</td>|
                );
            }
            $r->print(qq{</tr>});
        }
        ...
        @hash_assemblys = qw(
          ...
          assembly_part_currency    assembly_part_assembly_id
        );
        ...
        <strong class="">Assembly Parts</strong>
        </caption>
        <thead>
            <tr>
            <th align="left">Description</th>
        ...
        while ( $assem_part_hash_ref = $sth->fetchrow_hashref ) {
            ...
            # This substitution allows newlines to be converted to HTML code
                ...
                $r->print(
qq|<td align="left" class="assemblies_bl">$assem_part_hash_ref->{$assemblyh}</td>|
                );
            }
            $r->print(qq{</tr>});
        }
    ...
    @full_assembly_hash_assemblys = qw(
      full_assembly_name
    ...
    $sth = $dbh->prepare(
            "SELECT "
          . join( ",", @full_assembly_hash_assemblys )
          . " FROM full_assembly
              WHERE full_assembly_name=$full_assembly_list;"
    );
    $sth->execute();
    ...
    <caption class="FALcaption">
    <strong class="">Assemblies within Full Assembly</strong>
    </caption>
    <thead>
        <tr>
    ...
    while ( $full_assembly_hash_ref = $sth->fetchrow_hashref ) {
        ...
        foreach my $assemblyh (@full_assembly_hash_assemblys) {
            ...
            $r->print(
qq|<td align="left" class="full_assembly">$full_assembly_hash_ref->{$assemblyh}</td>|
            );
        }
        $r->print(qq{</tr>});
    }
    ...
    @full_assembly_list_hash_assemblys = qw(
      ...
      full_assembly_list_notes    full_assembly_list_id
    );
    $sth = $dbh->prepare(
            "SELECT "
          . join( ",", @full_assembly_list_hash_assemblys )
          . " FROM full_assembly_list
              WHERE full_assembly_list_name = $full_assembly_list;"
    );
    $sth->execute();
    $r->print(
        qq{
    ...
    while ( $full_assembly_list_hash_ref = $sth->fetchrow_hashref ) {
        ...
        foreach my $assemblyh (@full_assembly_list_hash_assemblys) {
            ...
            $r->print(
qq|<td align="left" class="full_assembly_list">$full_assembly_list_hash_ref->{$assemblyh}</td>|
            );
        }
        $r->print(qq{</tr>});
    }
...
<br /><br />
<form name="duplicates" class="duplicates" id="duplicates" action="$arg_ref->{Program}{program_path_name}" method="post">
<div>
    |
    );
...
<tr>
<td align="left">Re-Use This Assembly As-is</td><td align="left" class="assemblies">&nbsp;&nbsp;
<input class="assemblies" type="checkbox" name="assembly_Reuse$aund$a_c" id="assembly_Reuse$aund$a_c" value="assembly_Reuse$aund$a_c" />
</td>
</tr>}
            );
...
#######################################################################
    ...
    <tbody>}
        ...
        $statement =
"SELECT assembly_name FROM assemblies WHERE assembly_id = $$full_assembly_hash_ref{'full_assembly_assembly_id'};";
        $assembly_name_aref = $dbh->selectcol_arrayref($statement)
          || die $dbh->errstr;
...
<tr>
<td align="left">Re-use This Full Assembly As-is</td><td align="left" class="full_assembly">&nbsp;&nbsp;
<input class="full_assembly" type="checkbox" name="full_X_assembly_Reuse$fund2$f_a_c" id="full_X_assembly_Reuse$fund2$f_a_c" value="$$full_assembly_hash_ref{'full_assembly_assembly_id'}" />
</td>
</tr>
...
sub DuplicateFullFALRecords {
    ...
    for my $i ( 1 .. $#assembly_insert_assemblys ) {
        ...
        unless ( $assembly_insert_values[$i] =~ /DoNotInsert/
            || $assembly_insert_values[$i] =~ /Reuse/ )
        {
            $SQL =
qq{INSERT INTO assemblies ($assembly_insert_assemblys[$i]) VALUES ($assembly_insert_values[$i]) RETURNING assembly_id;};
            ...
            $assemblies_parts_insert_idassemblys[$i] =
              "assembly_part_assembly_id";
            $assemblies_parts_insert_ids[$i] = "DoNotInsert";
...
qq{INSERT INTO assemblies_parts ($assemblies_parts_insert_assemblys[$i][$j]) VALUES ($assemblies_parts_insert_values[$i][$j]) RETURNING assembly_part_id;};
                ...
                if ($num_rows) {
                    ...
                    $r->print(
                        qq{<div><h1>Failure -- $DBI::errstr</h1></div>} );
                }
            }
            else {
                $r->print(qq{<div><h1>Not Inserted</h1></div>});
    ...
    foreach my $key ( keys %args ) {
        ...
        if ( $key =~ /full_X_assembly_Reuse_full_assembly_count_(\d+)/ ) {
            $counted = $1;
            if ( ( defined $counted ) && ( $counted ne $old ) ) {
                push( @counted,   $counted );
                push( @added_ids, $args{$key} );

            }
            $old = $counted;
        }
    }
    foreach my $i ( 0 .. $#counted ) {
        $full_assembly_fields = '';
        $full_assembly_values = '';
        foreach my $key ( keys %args ) {
            if ( $key =~
                /(full_assembly.*)_full_assembly_count_($counted[$i])$/ )
            {
                unless ( $1 eq "full_assembly_assembly_quantity" ) {
                    ...
                    $full_assembly_fields .=
                      "full_assembly_assembly_subtotal, ";
                    $full_assembly_values .=
"(SELECT assembly_subtotal FROM assemblies WHERE assembly_id = $added_ids[$i]), ";
                    $full_assembly_fields .=
                      "full_assembly_assembly_rowtotal, ";
                    $full_assembly_values .=
"(SELECT (assembly_subtotal)*$args{$key} FROM assemblies WHERE assembly_id = $added_ids[$i]), ";
                }
                $full_assembly_fields .= "$1, ";
                $full_assembly_values .= "$quoted, ";

            }
        }
        $full_assembly_fields .= "full_assembly_assembly_id";
        $full_assembly_values .= $added_ids[$i];

        $SQL =
qq{INSERT INTO full_assembly ($full_assembly_fields) VALUES ($full_assembly_values) RETURNING full_assembly_id;};
        $r->print(qq{$SQL<br />});
...
qq{INSERT INTO full_assembly ($full_assembly_fields[$i]) VALUES ($full_assembly_values[$i]) RETURNING full_assembly_id;};
            $r->print(qq{$SQL<br />});
            $SQL =~ s/_full_assembly_count_//g;
            $r->print(qq{<br />XXX<br />$SQL substituted<br />});
            $sth      = $dbh->prepare($SQL);
            $num_rows = $sth->execute();
            if ($num_rows) {
                $new_full_assembly_id = $sth->fetch()->[0];
    ...
    foreach my $key ( keys %args ) {
        if (
            ...
            || ( $key =~
                /(full_assembly_list.*)_full_assembly_list_count_(\d+)$/ )
          )
        {
...
View full assembly list tree or duplicate to a similar tree.

=head1 DESCRIPTION

ViewFALRecords Allows viewing existing full_assembly_lists in light detail.
ViewFullFALRecords Allows viewing existing full_assembly_lists in full detail.
DuplicateFullFALRecordsForm Shows a form to create approximate copies of existing full_assembly_lists, allowing
changes and reuse of existing assemblies, assemblies_parts and full_assemblies.
Undesired parts, assemblies and full_assemblies can be removed individually.
It is not necessary to create a full_assembly_list or full_assembly with this program.
Since a full_assembly_list is always created from an existing full_assembly, only
the full_assembly_list_name field can change the full_assembly_name fields.
Assembly_part_description field is read-only since that field is derived from
product_description after insertion.
DuplicateFullFALRecords Performs the actual duplication and shows any errors.

=head1 BUGS AND LIMITATIONS

Only one set of empty assemblies, assemblies_parts and full_assemblies
is available.
This is a javascript limitation on my part and is fixable.
Javascript has a problem. Please make a small change to every assembly
and then reverse it if you want the same name values.
