...
sub Selecter {
    ...
    if ( $table eq "products" ) {
        ...
        @select_fields
            = qw(residential_or_commercial cust_bill_country);
        @field_names = qw(
        ...
        @select_fields
            = qw(cust_id residential_or_commercial jobsite_country);
        @field_names = qw(
        ...
        @select_fields
            = qw(full_assembly_list_name full_assembly_list_currency);
        @field_names = qw(
        ...
        @notes_fields
            = qw(labor_project_unit_notes labor_project_notes);
    ...
    if ( $sub eq "ViewTable" ) {
        push( @order,
            @field_names,     @specialty_fields, @select_fields,
            @no_quote_fields, @view_only,        @notes_fields,
            $table_id_field );
    ...
    if ( $sub eq "RecordUpdates" ) {
        push( @order,
            @field_names,     @select_fields, @specialty_fields,
            @no_quote_fields, @notes_fields );
    ...
    if ( $sub eq "InsertRecord" ) {
        my @order;
        push( @order,
            @field_names,  @specialty_fields,
            @notes_fields, @select_fields );
